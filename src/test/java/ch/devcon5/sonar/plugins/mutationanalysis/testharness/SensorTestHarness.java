...
public class SensorTestHarness {
  ...
  public Path createSourceFile(String path, String filename) {
  ...
  public Path resourceToFile(final String filePath, final String resource) {
    return this.folder.map(f -> {
      try {
  ...
  /**
   * @param ruleKeys The keys for the new rules profile
   * @return The new rules profile
  ...
  public ActiveRules createActiveRules(String... ruleKeys) {
    ...
    for (String ruleKey : ruleKeys) {
      builder.addRule(
          newActiveRule().setRuleKey(RuleKey.of(MutationAnalysisRulesDefinition.REPOSITORY_KEY + ".kotlin", ruleKey))
              .build());
      builder.addRule(
          newActiveRule().setRuleKey(RuleKey.of(MutationAnalysisRulesDefinition.REPOSITORY_KEY + ".java", ruleKey))
              .build());
    }
  ...
  public ActiveRules createActiveRules(Rule... rules) {
    final ActiveRulesBuilder builder = new ActiveRulesBuilder();
    for (Rule rule : rules) {
      final NewActiveRule.Builder activeRule = newActiveRule().setRuleKey(
          RuleKey.of(rule.getRepositoryKey(), rule.getKey()));
